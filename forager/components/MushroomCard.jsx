    ...
    <div>
      <div className="flex flex-col items-center">
        <div className="w-[100px] h-[129px] flex flex-col items-center bg-white shadow-md justify-start">
          ...
          >
            {/* Conditionally render the SVG only if toxic is true */}
            {toxic && (
              <svg
...
const MushroomCardPercent = ({ title, imageId, imageUrl, percent, toxic }) => {
  ...
  if (toxic) {
    if (percent >= 90) {
      squareColor = '#FF5050'; // 90+ and toxic
    } else if (percent >= 80) {
      squareColor = '#FF6666'; // 80+ and toxic
    } else {
      squareColor = '#FF7E7E'; // Less than 80 and toxic
    }
  } else {
    if (percent >= 60) {
      squareColor = 'rgba(165, 165, 165, 0.9)'; // 60+ and not toxic
    } else {
      squareColor = 'rgba(115, 216, 159, 0.9)'; // Less than 60 and not toxic
    }
  }
